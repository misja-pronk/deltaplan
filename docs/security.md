...
- **Identifiers are always quoted.** SQL is never assembled by concatenating raw
  identifiers; one `quote_ident()` helper handles every name that reaches a statement.
- **Safe by default.** See the [safety model](safety.md): only managed tables can be
  dropped, destructive steps need `--allow-destructive`, and a stale plan is refused.
